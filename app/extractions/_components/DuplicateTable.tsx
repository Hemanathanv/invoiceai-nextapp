...
const FileNameCellRenderer = (params: ICellRendererParams) => {
  ...
  const { duplicate } = data.statusCounts
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2">
        ...
        {data.page_count > 0 && data.file_name.toLowerCase().endsWith(".pdf") && (
          <Badge variant="default" className="text-xs bg-green-600">
            {data.page_count} pages Approved
          </Badge>
        )}
      ...
      <div className="flex items-center gap-2 pl-6">
        {duplicate > 0 && data.file_name.toLowerCase().endsWith(".pdf") && (
          <Badge variant="default" className="text-xs bg-green-600">
            {duplicate} Duplicate
          </Badge>
        )}
      ...
      {isExpanded && data.page_count > 0 && (
        <div className="ml-8 space-y-1 border-l-2 border-gray-200 pl-4">
          {data.pages
              .filter((page) => page.status === "duplicate")
              .map((page) => (
            <div
              key={page.id}
          ...
          {data.pages.filter((page) => page.status === "duplicate").length === 0 && (
...
const ActionsCellRenderer = (params: ICellRendererParams) => {
  ...
  const dupPages = data.pages.filter((p) => p.status === "duplicate")
  ...
  const firstHoldPage = dupPages[0]
...
export function DuplicateTable({
  ...
  const { data: fields} = useFieldHeaders(selectedClient);
  ...
  const duplicateInvoices = useMemo(() => {
    ...
    return invoices.data.map((group) => {
      const dupPages = group.pages.filter((p) => p.status === "duplicate");
      return {
        ...group,
        pages:             dupPages,
  ...
  const usedHeaderNames = useMemo(() => {
    ...
    for (const grp of duplicateInvoices) {
    ...
    const headerCols: ColDef[] = headerOrder
      ...
      .map((name) => ({
        ...
        valueGetter: (params) => {
          ...
          // Join unique values by commas, or return as array for custom renderer
          return uniqueValues;
        },
        cellRenderer: (params: ICellRendererParams) => {
          const values = params.value as string[];
          if (!values || values.length === 0) return null;
          const showAlert = values.length > 1;
          return (
            <div className="flex flex-col h-full">
              <ul className="list-disc ml-4 overflow-auto">
                {values.map((val, idx) => (
              ...
              {showAlert && (
                <span title="Multiple values detected"> 
                <AlertTriangle
                  className="self-start mt-auto h-4 w-4 text-yellow-500"
                />
                </span>
              )}
            </div>
  ...
  if (error) {
    return (
      <div className="text-center py-8">
        <p className="text-red-600">Error loading duplicate invoices: {error.message}</p>
      </div>
    )
  }
  ...
  if (!invoices?.data?.length) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-500">No duplicate invoices found</p>
        <p className="text-sm text-gray-400 mt-2">Duplicate invoices ready for export will appear here</p>
      </div>
    )
  }
