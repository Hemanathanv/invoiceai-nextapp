...
const FileNameCellRenderer = (params: ICellRendererParams) => {
  ...
  const { approved } = data.statusCounts
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2 pl-6">
        {approved > 0 && data.file_name.toLowerCase().endsWith(".pdf") && (
          <Badge variant="default" className="text-xs bg-green-600">
            {approved} Approved
          </Badge>
        )}
      ...
      {isExpanded && data.page_count > 0 && (
        <div className="ml-8 space-y-1 border-l-2 border-gray-200 pl-4">
          ...
          {data.pages.filter((page) => page.status === "approved").length === 0 && (
...
const ActionsCellRenderer = (params: ICellRendererParams) => {
  ...
  const approvedPages = data.pages.filter((p) => p.status === "approved")
  ...
  const firstHoldPage = approvedPages[0]
...
export function ApprovedTable({
  ...
  const approvedInvoices = useMemo(() => {
    ...
    return invoices.data.map((group) => {
      const approvedPages = group.pages.filter((p) => p.status === "approved");
      return {
        ...group,
        pages:             approvedPages,
        ...
        invoice_lineitems: approvedPages.flatMap((p) => p.invoice_lineitems ?? []),
      };
    });
  }, [invoices?.data]);
  ...
  const usedHeaderNames = useMemo(() => {
    ...
    for (const grp of approvedInvoices) {
  ...
  if (error) {
    return (
      <div className="text-center py-8">
        <p className="text-red-600">Error loading approved invoices: {error.message}</p>
      </div>
    )
  }
  ...
  if (!invoices?.data?.length) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-500">No approved invoices found</p>
        <p className="text-sm text-gray-400 mt-2">Approved invoices ready for export will appear here</p>
      </div>
    )
  }
